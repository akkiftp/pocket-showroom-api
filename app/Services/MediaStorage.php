...
class MediaStorage {
 public function uploadImage(UploadedFile $file,string $folder): string {
   ...
   if($url){
     $cloud=new Cloudinary(trim((string)$url," \t\n\r\0\x0B\"'"));
     $r=$cloud->uploadApi()->upload($file->getRealPath(),[
       'folder'=>'showmora/'.$folder,'resource_type'=>'image','overwrite'=>false
     ]);
     return (string)$r['secure_url'];
   }
 ...
 public function delete(?string $value): void {
   if(!$value || str_starts_with($value,'http')) return;
